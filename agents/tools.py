...
def get_embeddings():
    # Force Ollama embeddings for now because that's what we used for indexing
...
def find_references(symbol_name: str):
    ...
    project = jedi.Project(".")
    # Using search and then references for each hit might be slow, 
    # but jedi's Project.search is a good entry point.
    names = project.search(symbol_name)
    results = []
    for name in names:
        if name.is_definition():
    ...
    # Limit results to top 15 to avoid context blowup
...
def list_files(directory: str = "."):
    ...
    files_list = []
    ignore_list = [".git", "__pycache__", ".venv", "chroma", "indexing", ".gemini", ".pytest_cache", ".venv"]
    ...
    return "\n".join(files_list[:100]) # Limit to 100 files for context safety

# More tools like get_diagnostics (LSP) can be added here later
tools = [search_code, read_file, get_symbol_info, find_references, list_files]
